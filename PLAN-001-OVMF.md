...
1. Add a host-visible graphical path for OVMF's ramfb/GOP output.  It feeds a
   bounded staging surface and a separate RFB worker over a mode-0600 Unix
   socket; the display worker must not map arbitrary guest RAM or receive
   `/dev/vmm`.
2. Use an unmodified Windows installer as the first graphical integration test:
   firmware and Setup must be visible and accept input even though virtio
   storage is not yet visible.  Driver-injected WIM installation is a separate
   subsequent milestone.
3. Exercise EFI SMP, MSI-X, and networking with installed OpenBSD and Linux
   guests, followed by Windows installation media.  Confirm OpenBSD's
   `machine fw` flow as a guest-level check of the already-tested EFI reset
   service.
4. Add focused runtime coverage for malformed variable stores, ephemeral
   versus persistent variables, CFI flash bounds, and ACPI handoff.
5. Add a power-failure-safe persistence protocol if EFI variable durability
...
6. Only after the non-SMM path is stable, evaluate Secure Boot, authenticated
...
- SMBIOS is generated by vmd and published through
  `etc/smbios/smbios-tables`.  OVMF consumes that structure stream and creates
  its own EFI SMBIOS entry point; SeaBIOS additionally consumes
  `etc/smbios/smbios-anchor` when installing the legacy-visible entry point.
